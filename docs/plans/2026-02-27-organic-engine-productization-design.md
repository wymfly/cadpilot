...
```
data: {"status": "created", "job_id": "xxx"}
data: {"status": "analyzing", "message": "正在分析创意需求…"}
data: {"status": "generating", "message": "AI 正在生成 3D 模型…", "provider": "tripo3d", "progress": 0.3}
data: {"status": "generating", "message": "3D 模型生成完成", "progress": 1.0}
data: {"status": "post_processing", "message": "正在修复网格…", "step": "repair"}
data: {"status": "post_processing", "message": "正在缩放到目标尺寸…", "step": "scale"}
data: {"status": "post_processing", "message": "正在切削工程接口…", "step": "boolean"}
data: {"status": "post_processing", "message": "正在校验网格质量…", "step": "validate"}
data: {"status": "completed", "model_url": "/outputs/xxx/model.glb", "mesh_stats": {...}}
```
...
POST /api/generate/organic/upload   # 图片上传入口（Image-to-3D）→ SSE 流
...
class EngineeringCut(BaseModel):
    """工程接口切削定义"""
    type: Literal["flat_bottom", "hole", "slot"]
    diameter: float | None = None       # mm (hole)
    depth: float | None = None          # mm
    width: float | None = None          # mm (slot)
...
class OrganicGenerateRequest(BaseModel):
    ...
    constraints: OrganicConstraints = OrganicConstraints()
...
> **注意：** PyMeshLab 为 GPL v3 许可。如果项目需要闭源发布，需要评估 GPL 传染性问题，或改用 manifold3d 自带的修复能力替代。
